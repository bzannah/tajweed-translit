...
export async function generateStaticParams() {
  return [{ number: "015", slug: "al-hijr" }];
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata {
  ...
  return {
    title: "Surah Al-Hijr (الحجر) — The Rocky Tract | TajweedTranslit",
    description: "Read Surah Al-Hijr (The Rocky Tract) — 99 verses, Meccan revelation. English transliteration with expert Tajweed colour coding. Free online reader.",
    keywords: [
      "Surah Al-Hijr transliteration",
      "Surah 15 tajweed",
      "Al-Hijr meaning",
      "Quran al-hijr transliteration",
      "learn quran al-hijr",
      "quran recitation al-hijr",
      "tajweed al-hijr",
    ],
    alternates: { canonical: `/surah/15/al-hijr` },
    openGraph: {
      title: "Surah Al-Hijr (الحجر) — The Rocky Tract",
      description: "Read Surah Al-Hijr in English transliteration with Tajweed colour coding. 99 verses, Meccan.",
      url: `/surah/15/al-hijr`,
      type: "article",
    },
  };
}
    ...
    <div className="min-h-screen bg-background text-foreground"
      ...
      <nav className="border-b border-border px-4 py-3 text-sm" aria-label="Breadcrumb"
        <ol className="flex items-center gap-2 text-muted"
          ...
          <li/</li
          <li className="text-foreground" aria-current="page"Surah Al-Hijr</li
        </ol
      </nav
      ...
      <header className="px-4 py-12 text-center"
        ...
        <h1 className="font-brand text-4xl text-accent mb-2"الحجر</h1
        <h2 className="font-brand text-2xl text-foreground mb-1"Surah Al-Hijr</h2
        <p className="text-lg text-secondary italic mb-4"The Rocky Tract</p
        ...
        <div className="mt-8"
          <Link
            ...
            Read Surah Al-Hijr with Tajweed →
          </Link
          <p className="mt-2 text-xs text-muted"
            Colour-coded transliteration · Page 527
          </p
        </div
      </header
      ...
      <div className="mx-auto max-w-4xl px-4 pb-16 space-y-12"
        ...
        {/* About */}
        <section
          <h3 className="font-brand text-xl text-accent mb-4"About Surah Al-Hijr</h3
          <p className="text-secondary leading-relaxed"Named after the stone valley of the Thamud people who rejected their prophet Salih.</p
        </section

        {/* Significance */}
        <section className="rounded-xl border border-border bg-surface p-6"
          <h3 className="font-brand text-lg text-accent mb-3"Why Surah Al-Hijr is Special</h3
          <p className="text-secondary leading-relaxed"Contains the verse about the Quran being a confirmation of previous scriptures.</p
        </section
        ...
        <section
          ...
          <dl className="grid grid-cols-2 gap-4 text-sm"
            <div className="rounded-lg border border-border bg-surface p-4"
              ...
              <dd className="mt-1 text-lg font-semibold text-foreground"15</dd
            </div
            <div className="rounded-lg border border-border bg-surface p-4"
              <dt className="text-muted text-xs uppercase tracking-wide"Verses</dt
              <dd className="mt-1 text-lg font-semibold text-foreground"99</dd
            </div
            <div className="rounded-lg border border-border bg-surface p-4"
            ...
            <div className="rounded-lg border border-border bg-surface p-4"
              ...
              <dd className="mt-1 text-lg font-semibold text-foreground italic"The Rocky Tract</dd
            </div
            <div className="rounded-lg border border-border bg-surface p-4 col-span-2"
              ...
              <dd className="mt-1 text-lg font-semibold text-foreground"
                <Link href="/page/527" className="text-accent hover:underline"
                  Page 527 →
                </Link
              </dd
            </div
        ...
        <section
          ...
          <p className="text-sm text-secondary mb-4"
            When reading Surah Al-Hijr at TajweedTranslit.com, these Tajweed rules are colour-coded:
          </p
        ...
        <section className="grid grid-cols-2 gap-4"
          <Link href="/surah/14/ibrahim" className="rounded-lg border border-border bg-surface p-4 text-center hover:border-accent transition-colors"
            <span className="text-xs text-muted"← Previous</span
            <p className="mt-1 font-semibold text-foreground"Surah Ibrahim</p
          </Link
